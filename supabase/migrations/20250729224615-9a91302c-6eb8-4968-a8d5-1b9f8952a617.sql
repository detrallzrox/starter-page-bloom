...
DO $$
BEGIN
    PERFORM extensions.cron.unschedule('process-daily-notifications');
EXCEPTION
    WHEN OTHERS THEN
        NULL;
END $$;
...
-- 7. Adicionar constraints únicas se não existirem
DO $$
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'subscribers_user_id_unique') THEN
